...
class Att(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkvc = qkvc.contiguous().view(
            b, self.num_patch * self.num_patch, self.inner_dim * 4)
        ...
        q, k, v, c = map(
            lambda t: einops.rearrange(
                t, "b n (h d) -> b h n d", h=self.num_heads), qkvc)
...
class Encoder_Transformer(nn.Module):
    ...
    def __init__(self, dim: int, num_layers: int, num_heads: int,
                 head_dim: int, hidden_dim: int, num_patch: int, patch_size: int):
        super().__init__()
        ...
        for _ in range(num_layers):
            layer = [
                ...
                nn.Sequential(
                    nn.LayerNorm(dim), Att(
                        dim, head_dim, num_heads, num_patch, patch_size)),
                nn.Sequential(
                    nn.LayerNorm(dim),
                    Conv(
                        dim,

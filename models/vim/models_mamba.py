        ...
        if if_abs_pos_embed:
            self.pos_embed = nn.Parameter(
                torch.zeros(
                    1,
                    channels + 1,
                    self.embed_dim))  # num_patches + self.num_tokens
        ...
        if if_random_token_rank:
            ...
            if isinstance(token_position, list):
                print("original value: ",
                      x[0, token_position[0], 0], x[0, token_position[1], 0])
            else:
            ...
            if isinstance(token_position, list):
                ...
                new_token_position = [
                    torch.where(
                        shuffle_indices == token_position[i])[0].item() for i in range(
                        len(token_position))]
                token_position = new_token_position
            else:
                # 找到 cls token 在 shuffle 之后的新位置
                token_position = torch.where(
                    shuffle_indices == token_position)[0].item()

...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        # dout_y = rearrange(dout, "b l d -> b d l") # because no arrange at
        # end of forward, so dout shape is b d l
...
def mamba_inner_ref(
    ...
    # We're being very careful here about the layout, to avoid extra transposes.
    # We want delta to have d as the slowest moving dimension
    # and L as the fastest moving dimension, since those are what the ssm_scan
    # kernel expects.
...
def bimamba_inner_ref(
    ...
    y = selective_scan_fn(
        ...
        delta_bias=delta_bias,
        delta_softplus=True)
    y_b = selective_scan_fn(x.flip([-1]),
                            delta.flip([-1]),
                            A_b,
                            B.flip([-1]),
                            C.flip([-1]),

...
def min_max_norm_val(hsi_path, gt_path, imageList, channels):
    ...
    for imgID in imageList:
        ...
        labels = np.squeeze(np.load(gt_path + imgID + '.npy'))
...
def get_cube_and_GT(idp, data_path, gt_path, patch_size, minMaxVects):
    ...
    gt = np.squeeze(np.load(f"{gt_path}{idp}.npy"))
    # gt = np.array(Image.open(f'{gt_path}{idp}.png'), dtype=np.uint8)
...
def loadImagesData(hsi_path, gt_path, imglist, patch_size,
    ...
    for imgID in imglist:
        hsi_dataset = np.load(hsi_path + imgID + '.npy')
        labels = np.squeeze(np.load(gt_path + imgID + '.npy').astype(np.int8))
    ...
    """ SHUFFLE DATA """
    indx = list(range(0, data_samps.shape[0]-1))
    random.Random(4).shuffle(indx)

    data_samps = data_samps[indx,...]
    gt_labs = gt_labs[indx]
    """ """

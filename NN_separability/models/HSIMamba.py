...
class HSIVimBlock(nn.Module):
    def __init__(self, spatial_dim, num_bands, hidden_dim,
                 output_dim, delta_param_init):
        ...
        # LayerNorm is now expecting a flattened feature vector of Bands*H*W
        # elements
        ...
        self.delta_param = nn.Parameter(
            torch.full((hidden_dim,), delta_param_init))

        self.linear_forward = nn.Linear(hidden_dim, output_dim)
        self.linear_backward = nn.Linear(hidden_dim, output_dim)

    def forward(self, x):
        # Correct shape extraction assuming [Batch, Height, Width, Bands]
        Batch, H, W, Bands = x.shape

        # Correctly reshape for LayerNorm to flatten all spatial and spectral
        # information
        ...
        # Bidirectional Conv1d processing using reversed input for the backward
        # path
        F.silu(self.forward_conv1d(x_proj))
        F.silu(self.backward_conv1d(z_proj_reversed))

        # Apply delta parameter correctly
        delta_expanded = self.delta_param.unsqueeze(
            0).unsqueeze(2)  # Correct shape for broadcasting

        # SSM processing with delta applied, using the original and reversed
        # inputs for forward and backward paths respectively
        forward_ssm_output = torch.tanh(
            self.forward_conv1d(x_proj) + self.A * delta_expanded)
        backward_ssm_output = torch.tanh(
            self.backward_conv1d(z_proj_reversed) + self.B * delta_expanded)
...
class SpatialFeatureProcessing(nn.Module):
    def __init__(self, input_channels):
        ...
        self.conv_layers = nn.Sequential(
            # First convolutional layer with dilation rate of 1 (standard
            # convolution)
            nn.Conv2d(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(256),
            # Second convolutional layer with a higher dilation rate to
            ...
            nn.Conv2d(
                ...
                padding=2,
                dilation=2),
            # Note the increased padding to maintain the spatial dimensions
            nn.ReLU(),
            nn.BatchNorm2d(512)
        )
        self.global_avg_pool = nn.AdaptiveAvgPool2d(
            (1, 1))  # Adding global average pooling
...
class Classifier(nn.Module):
    ...
    def forward(self, x):
        x = self.fc_layers(x)
        # Remove softmax here if you're using a loss function that includes it,
...
class HSIClassificationMambaModel(nn.Module):
    def __init__(self, spatial_dim, num_bands, hidden_dim,
                 output_dim, delta_param_init, num_classes):
        super(HSIClassificationMambaModel, self).__init__()
        self.vim_block = HSIVimBlock(
            spatial_dim,
            num_bands,
            hidden_dim,
            output_dim,
        ...
        # Adjusted to pass 'output_dim' as 'input_channels' to
        # SpatialFeatureProcessing
        self.spatial_processing = SpatialFeatureProcessing(
            input_channels=output_dim)
        # Assuming the output of SpatialFeatureProcessing matches the
        # in_features expected by Classifier
        self.classifier = Classifier(in_features=512, num_classes=num_classes)

    def forward(self, x):
        x = self.vim_block(x)
        # This is a placeholder. Actual reshaping depends on the output of
        # HSIVimBlock and the input expectation of SpatialFeatureProcessing
        # Reshape to include spatial dimensions if needed
        x = x.contiguous().view(-1, self.output_dim, 1, 1)

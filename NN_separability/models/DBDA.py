...
class DBDA(nn.Module):
    def __init__(self, band=25, classes=4):
    ...
    def sep_forward(self, X, Y):
        ...
        sep_indices.append(compute_sep(x11,Y))

        # print('x11', x11.shape)
        x12 = self.batch_norm11(x11)
        x12 = self.conv12(x12)

        sep_indices.append(compute_sep(x12,Y))
        # print('x12', x12.shape)
        ...
        sep_indices.append(compute_sep(x13,Y))

        x14 = torch.cat((x11, x12, x13), dim=1)
        x14 = self.batch_norm13(x14)
        x14 = self.conv14(x14)

        sep_indices.append(compute_sep(x14,Y))
        ...
        sep_indices.append(compute_sep(x16,Y))
        ...
        sep_indices.append(compute_sep(x24,Y))
        ...
        sep_indices.append(compute_sep(output,Y))

        # output = self.fc(x_pre)
        return output, sep_indices
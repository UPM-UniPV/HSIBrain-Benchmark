...
class DenseEasyAttn(nn.Module):
    ...
    def split_heads(self, x):
        ...
        return x.contiguous().view(batch_size, seq_length,
...
class TBlock(nn.Module):
    def __init__(self, embedDim, heads, mlp_dim,
                 channels, easyAtt, dropout, dropPath):
        super().__init__()
...
class ViT (nn.Module):
    def __init__(self, patchSize=7, nBlocks=5, mlp_dim=4, numHeads=16, caf=False,
                 easyAtt=False, embedDim=64, numClasses=4, dropout=0.1, dropPath=0.1, channels=25):
        super().__init__()
        ...
        # Initialization of the additional embeddings using a truncated normal
        # distribution
        ...
        self.blocks = nn.ModuleList([TBlock(embedDim,
                                            numHeads,
                                            mlp_dim,
                                            channels,
                                            easyAtt,
        ...
        if self.caf is not False:
            ...
            for _ in range(nBlocks - 2):
                self.skipcat.append(
                    nn.Conv2d(channels + 1, channels + 1, [1, 2], 1, 0))
    ...
    def sep_forward(self, px, Y):

        sep_indices = []

        sep_indices.append(compute_sep(px,Y))

...
class SpatialAttentionModule(nn.Module):
    ...
    def forward(self, X):
        ...
        x_att = torch.cat(
            (X.mean(1).unsqueeze(1), torch.amax(
                X, dim=1).unsqueeze(1)), dim=1)
...
class RSSAN(nn.Module):
    ...
    def __init__(self, in_dims=1, in_chns=25, embed_dim=32,
                 patch_size=11, out_classes=4):
    ...
    def sep_forward(self, X, Y):
        ...
        sep_indices.append(compute_sep(x_se,Y))


        x_sa = self.SaAM(x_se)

        sep_indices.append(compute_sep(x_sa,Y))

        xc = self.conv_block(x_sa)

        sep_indices.append(compute_sep(xc,Y))

        xc = self.SSFL_block[0](xc)

        sep_indices.append(compute_sep(xc,Y))
        
        xc = self.SSFL_block[1](xc)

        sep_indices.append(compute_sep(xc,Y))

        xc = self.SSFL_block[2](xc)
        x_out = self.SSFL_block[3](xc)

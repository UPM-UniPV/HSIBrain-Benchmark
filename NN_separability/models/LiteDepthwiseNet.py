...
class LiteDwNet(nn.Module):
    ...
    def __init__(self, in_dims=1, in_chns=25, patch_size=11, out_classes=4):
        ...
        self.input_stem = nn.Sequential(nn.Conv3d(in_dims, 24, (7, 1, 1), stride=(2,1,1), bias=False),
                                        nn.BatchNorm3d(24),
    ...
    def sep_forward(self, X, Y):
        ...
        x_l = self.left_branch[0](x)

        sep_indices.append(compute_sep(x,Y))

        x_l = self.left_branch[1](x_l)


        sep_indices.append(compute_sep(x_l,Y))


        x_r = self.right_branch[0](x)

        sep_indices.append(compute_sep(x_r,Y))

        x_r = self.right_branch[1](x_r)

        sep_indices.append(compute_sep(x_r,Y))

        x_r = self.right_branch[2](x_r)

        sep_indices.append(compute_sep(x_r,Y))

        x_cat = torch.cat((x_l, x, x_r), dim=1)

        x_out = self.joint_branch[0](x_cat)
        x_out = self.joint_branch[1](x_out)

        sep_indices.append(compute_sep(x_out,Y))

        x_out = self.joint_branch[2](x_out)

        sep_indices.append(compute_sep(x_out,Y))

        return x_out, sep_indices
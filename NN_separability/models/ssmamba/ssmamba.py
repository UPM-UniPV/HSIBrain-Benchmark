...
from models.ssmamba.mamba import SSM
from models.ssmamba.pos_embed import get_2d_sincos_pos_embed
...
class spectral_spatial_block(nn.Module):
    def __init__(self, embed_dim, bi=False, N=8, drop_path=0.0,
                 norm_layer=nn.LayerNorm, cls=True, fu=True):
...
class PatchEmbed_1D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=2,
                 in_chans=1, embed_dim=64, norm_layer=None):
        super().__init__()
...
class PatchEmbed_2D(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=16,
                 in_chans=3, embed_dim=64, norm_layer=None, flatten=True):
        super().__init__()
...
class PatchEmbed_Spe(nn.Module):
    ...
    def __init__(self, img_size=(9, 9), patch_size=2,
...
class mamba_1D_model(nn.Module):
    ...
    def __init__(self, img_size=(3, 3), spa_img_size=(224, 224), nband=107, patch_size=1, embed_dim=128,
                 nclass=10, drop_path=0.0, depth=4, bi=True, norm_layer=nn.LayerNorm, global_pool=True, cls=True):
        super().__init__()
        self.patch_embed = PatchEmbed_Spe(
            img_size, patch_size, embed_dim, norm_layer)
...
class mamba_2D_model(nn.Module):
    def __init__(self, img_size=(224, 224), patch_size=16, in_chans=3, hid_chans=32, embed_dim=128,
                 nclass=10, drop_path=0.0, depth=4, bi=True, norm_layer=nn.LayerNorm, global_pool=True, cls=True):
        super().__init__()
        ...
        self.patch_embed = PatchEmbed_2D(
            img_size, patch_size, hid_chans, embed_dim)
...
class mamba_SS_model(nn.Module):
    def __init__(self, spa_img_size=(224, 224), spe_img_size=(5, 5), spa_patch_size=16, spe_patch_size=2, in_chans=3, hid_chans=64, embed_dim=64, nclass=10, drop_path=0.0, depth=4, bi=True,
        ...
        self.spe_patch_embed = PatchEmbed_Spe(
            img_size=spe_img_size,
            patch_size=spe_patch_size,
            embed_dim=embed_dim)
        self.spa_patch_embed = PatchEmbed_2D(
            spa_img_size, spa_patch_size, hid_chans, embed_dim)
    ...
    def initialize_weights(self):
        # initialization
        # initialize (and freeze) pos_embed by sin-cos embedding
        spa_pos_embed = get_2d_sincos_pos_embed(
            self.spa_pos_embed.shape[-1], int(self.spa_patch_embed.num_patches**.5), cls_token=True)
        self.spa_pos_embed.data.copy_(
            torch.from_numpy(spa_pos_embed).float().unsqueeze(0))
    ...
    def forward_features(self, x):
        ...
        if self.global_pool:
            x_spa = x_spa[:, 0:self.cs, :].mean(
                dim=1)  # global pool without cls token

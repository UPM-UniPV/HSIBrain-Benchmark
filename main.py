...
def main(args):
    ...
    if args.db_name == 'madrid':
        cube_dims = (217, 409, 25)
    elif args.db_name == 'LP':
        cube_dims = (370, 460, 128)
    else:
    ...
    min_vect, max_vect = tools.min_max_norm_val(args.data_path, args.gt_path, train_val_ids, cube_dims)
    ...
    if tools.is_main_process():
        if args.distributed:
            ...
            model_to_save = model
        X_sample = torch.randn(1, cube_dims[2], args.patch_size, args.patch_size).to(device)
    ...
    for test_image in test_ids:
        hsi, gt = tools.get_cube_and_GT(test_image, args.data_path, args.gt_path, patch_size=args.patch_size, minMaxVects=[min_vect, max_vect])
        ...
        if tools.is_main_process():
            ...
            data_reshaped = np.reshape(test_preds.cpu().numpy(), (cube_dims[0], cube_dims[1]))
            npimg = tools.getImage(data_reshaped, cube_dims[0], cube_dims[1])                                      
